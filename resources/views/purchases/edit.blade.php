@extends('layouts.main', ['title' => __('general.edit_purchase')])

@section('content')
    <div class="page-content">

        <div class="page-header">
            <h1>@lang('general.edit_purchase')</h1>
            <p>{{ $data->code }}</p>
        </div>

        <form action="{{ route('purchases.update', ['encryptedId' => $encryptedId]) }}" method="POST" id="purchase-form">
            @csrf
            @method('PATCH')
                ...
                <div class="flex flex-col gap-6 p-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <x-form.field name="purchase_date" :label="__('general.purchase_date')" :required="true">
                            <input type="date"
                                   name="purchase_date"
                                   value="{{ old('purchase_date', $data->purchase_date?->format('Y-m-d')) }}"
                    ...
                    <div class="flex items-center gap-4 text-sm w-full justify-end">
                        ...
                        <div class="w-44">
                            <input type="number"
                                   ...
                                   max="100"
                                   placeholder="0"
                                   value="{{ old('tax_percent', $data->tax_percent) }}"
                                   autocomplete="off"
