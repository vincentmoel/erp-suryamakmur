@extends('layouts.main', ['title' => __('general.add_receipt')])

@section('content')
    <div class="page-content">

        <div class="page-header">
            <h1>@lang('general.add_receipt')</h1>
            <p>@lang('general.add_receipt_desc')</p>
        </div>

        <form action="{{ route('receipts.store') }}" method="POST" enctype="multipart/form-data" id="receipt-form">
            @csrf
...
(function () {
    ...
    function showState(state) {
        ...
        document.getElementById('allocation-' + state).classList.remove('hidden');
        if (state === 'table-wrap') tableWrap.classList.remove('hidden');
    }
    ...
    function loadInvoices(customerId) {
        if (!customerId) { showState('placeholder'); return; }
        showState('loading');
        var url = ajaxUrl.replace('__CID__', customerId);
        fetch(url)
